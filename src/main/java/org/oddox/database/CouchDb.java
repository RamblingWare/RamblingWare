...
public class CouchDb extends DatabaseService {
	...
	public List<Author> getAuthors(int page, int limit, boolean includeAdmins) {
		...
		try {
			...
			ViewResponse<String, Object> pg = db.getViewRequestBuilder("authors", "authors")
					.newPaginatedRequest(Key.Type.STRING, Object.class).rowsPerPage(limit).includeDocs(true).build()
					.getResponse();
			...
			for (int i = 1; i < page; i++) {
				if (pg.getNextPageToken() != null) {
					...
					pg = db.getViewRequestBuilder("authors", "authors")
							.newPaginatedRequest(Key.Type.STRING, Object.class).rowsPerPage(limit).includeDocs(true)
							.build().getResponse(pg.getNextPageToken());
				} else {
					// page does not exist
					return authors;
				}
	...
	public List<Year> getYears() {
		...
		try {
			...
			ViewResponse<String, Integer> pg = db.getViewRequestBuilder("posts", "year-count")
					.newPaginatedRequest(Key.Type.STRING, Integer.class).group(true).build().getResponse();
	...
	public List<Post> getPosts(int page, int limit, boolean includeHidden) {
		...
		try {
			...
			ViewResponse<String, Object> pg = db.getViewRequestBuilder("posts", view)
					.newPaginatedRequest(Key.Type.STRING, Object.class).rowsPerPage(limit).descending(true)
					.includeDocs(true).build().getResponse();

			for (int i = 1; i < page; i++) {
				if (pg.getNextPageToken() != null) {
					// next page
					pg = db.getViewRequestBuilder("posts", view).newPaginatedRequest(Key.Type.STRING, Object.class)
							.rowsPerPage(limit).descending(true).includeDocs(true).build()
	...
	public List<Post> getPostsByCategory(int page, int limit, String category, boolean includeHidden) {
		...
		try {
			...
			ViewResponse<String, String> pg = db.getViewRequestBuilder("posts", view)
					.newPaginatedRequest(Key.Type.STRING, String.class).rowsPerPage(limit).reduce(false)
					.descending(true).keys(category).includeDocs(true).build().getResponse();

			for (int i = 1; i < page; i++) {
				if (pg.getNextPageToken() != null) {
					// next page
					pg = db.getViewRequestBuilder("posts", view).newPaginatedRequest(Key.Type.STRING, String.class)
							.rowsPerPage(limit).reduce(false).descending(true).keys(category).includeDocs(true).build()
							.getResponse(pg.getNextPageToken());
				} else {
					// page does not exist
					return posts;
	...
	public List<Post> getPostsByTag(int page, int limit, String tag, boolean includeHidden) {
		...
		try {
			...
			ViewResponse<String, String> pg = db.getViewRequestBuilder("posts", view)
					.newPaginatedRequest(Key.Type.STRING, String.class).rowsPerPage(limit).reduce(false)
					.descending(true).keys(tag).includeDocs(true).build().getResponse();

			for (int i = 1; i < page; i++) {
				if (pg.getNextPageToken() != null) {
					// next page
					pg = db.getViewRequestBuilder("posts", view).newPaginatedRequest(Key.Type.STRING, String.class)
							.rowsPerPage(limit).reduce(false).descending(true).keys(tag).includeDocs(true).build()
							.getResponse(pg.getNextPageToken());
				} else {
					// page does not exist
					return posts;
	...
	public List<Post> getPostsByYear(int page, int limit, int year, boolean includeHidden) {
		...
		try {
			...
			ViewResponse<String, String> pg = db.getViewRequestBuilder("posts", view)
					.newPaginatedRequest(Key.Type.STRING, String.class).rowsPerPage(limit).reduce(false)
					.descending(true).startKey(startKey).endKey(endKey).includeDocs(true).build().getResponse();

			for (int i = 1; i < page; i++) {
				if (pg.getNextPageToken() != null) {
					// next page
					pg = db.getViewRequestBuilder("posts", view).newPaginatedRequest(Key.Type.STRING, String.class)
							.rowsPerPage(limit).reduce(false).descending(true).startKey(startKey).endKey(endKey)
