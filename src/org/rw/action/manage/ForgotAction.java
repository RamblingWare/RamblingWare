	...
	public String execute() {
		...
		if (remind || reset) {
			...
			if (email != null && ApplicationStore.isValidEmail(email)) {
				...
				System.out.println("User failed to recover. Invalid Email was entered " + email + " (" + attempts + "/"
			...
			if (code != null && code.equalsIgnoreCase("123456")) {
				...
				System.out.println("User tried to recover with an invalid Recovery Code. (" + attempts + "/"
						+ maxAttempts + ") (" + servletRequest.getRemoteAddr() + ")");
				return ERROR;
			}
		}
